...
static void video_image_display(VideoState *is)
{
    ...
    if (vp->bmp) {
		...
		//av_log(NULL, AV_LOG_ERROR, "%s,%d, is->xleft:%d is->ytop:%d is->width:%d is->height:%d vp->width:%d vp->height:%d vp->sar:%d\n",__FUNCTION__,__LINE__,
...
static int configure_video_filters(AVFilterGraph *graph, VideoState *is, const char *vfilters, AVFrame *frame)
{
	...
	av_log(NULL, AV_LOG_ERROR, "%s,%d before create filter :%p\n",__FUNCTION__,__LINE__,avfilter_get_by_name("buffer"));
...
static int stream_component_open(VideoState *is, int stream_index)
{
    ...
    av_log(NULL, AV_LOG_FATAL, "stream_index: %d\n", stream_index);
    if (stream_index < 0 || stream_index >= ic->nb_streams)
        return -1;
    avctx = ic->streams[stream_index]->codec;

    codec = avcodec_find_decoder(avctx->codec_id);
    av_log(NULL, AV_LOG_FATAL, "avctx->codec_type: %d\n", avctx->codec_type);
...
static int read_thread(void *arg)
{
    ...
    av_log(NULL, AV_LOG_FATAL, "%s,%d st_index:%d\n",__FUNCTION__, __LINE__, st_index[AVMEDIA_TYPE_VIDEO] );
    if (!audio_disable)
    ...
    ret = -1;
    //av_log(NULL, AV_LOG_FATAL, "%s,%d is->video_stream:%d, is->audio_stream:%d\n, st_index:%d\n",__FUNCTION__, __LINE__, is->video_stream ,is->audio_stream, st_index[AVMEDIA_TYPE_VIDEO] );

    if (st_index[AVMEDIA_TYPE_VIDEO] >= 0) {
        ret = stream_component_open(is, st_index[AVMEDIA_TYPE_VIDEO]);
    }

    //av_log(NULL, AV_LOG_FATAL, "%s,%d is->video_stream:%d, is->audio_stream:%d\n",__FUNCTION__, __LINE__, is->video_stream ,is->audio_stream);
...
int main(int argc, char **argv)
{
    ...
    av_log(NULL, AV_LOG_FATAL, "%s, %d, kanlihu looper begin", __FUNCTION__, __LINE__);
    event_loop(is);
    av_log(NULL, AV_LOG_FATAL, "%s, %d, kanlihu looper end", __FUNCTION__, __LINE__);

                        ...
                        AVInputFormat *fmt, AVDictionary **options)
{
    ...
    av_log(NULL, AV_LOG_ERROR, "\n%s, %d, s->streams[0]->need_parsing:%d\n", __FUNCTION__,__LINE__, s->streams[0]->need_parsing);
    av_log(NULL, AV_LOG_ERROR, "\n%s, %d, s->streams[0]->parser:%p\n", __FUNCTION__,__LINE__, s->streams[0]->parser);
#if 0
    if (id3v2_extra_meta) {
...
static int read_frame_internal(AVFormatContext *s, AVPacket *pkt)
{
    ...
    while (!got_packet && !s->parse_queue) {
        ...
        //av_log(NULL, AV_LOG_FATAL, "%s,%d cur_pkt.size:%d\n",__FUNCTION__, __LINE__,cur_pkt.size);
        if (ret < 0) {
        ...
        //av_log(NULL, AV_LOG_FATAL, "%s,%d cur_pkt.stream_index:%d\n",__FUNCTION__, __LINE__, cur_pkt.stream_index);
        ...
        //av_log(NULL, AV_LOG_FATAL, "%s,%d st->need_parsing:%d,st->parser:%d\n",__FUNCTION__, __LINE__, st->need_parsing, st->parser);
        if (!st->need_parsing || !st->parser) {
            ...
            *pkt = cur_pkt;
            ...
            if ((ret = parse_packet(s, &cur_pkt, cur_pkt.stream_index)) < 0)
                return ret;
            //av_log(NULL, AV_LOG_FATAL, "%s,%d s->parse_queue:%p\n",__FUNCTION__, __LINE__, s->parse_queue);
        } else {
            /* free packet */
    ...
    //av_log(NULL, AV_LOG_FATAL, "%s,%d pkt->size:%d\n",__FUNCTION__, __LINE__,pkt->size);
...
/*            if (!st->time_base.num)
        ...
        //av_log(NULL, AV_LOG_ERROR, "\n%s, %d, ic->streams[0]->parser:%p\n", __FUNCTION__,__LINE__, ic->streams[0]->parser);
        //av_log(NULL, AV_LOG_ERROR, "\n%s, %d, !(ic->flags & AVFMT_FLAG_NOPARSE):%d\n", __FUNCTION__,__LINE__, !(ic->flags & AVFMT_FLAG_NOPARSE));
        if (!st->parser && !(ic->flags & AVFMT_FLAG_NOPARSE)) {
            st->parser = av_parser_init(st->codec->codec_id);
            //av_log(NULL, AV_LOG_ERROR, "\n%s, %d, st->parser:%p\n", __FUNCTION__,__LINE__, st->parser);
            if (st->parser) {
    ...
    for (;;) {
         ...
         * (AVFMTCTX_NOHEADER). */
        //av_log(NULL, AV_LOG_FATAL, "%s,%d pkt1.size:%d\n",__FUNCTION__, __LINE__,pkt1.size);
                        ...
                        AVCodec **decoder_ret, int flags)
{
    ...
    for (i = 0; i < nb_streams; i++) {
        ...
        if (decoder_ret) {
            ...
            av_log(NULL, AV_LOG_FATAL, "%s,%d decoder:%p\n",__FUNCTION__, __LINE__, decoder);
            if (!decoder) {
                if (ret < 0)

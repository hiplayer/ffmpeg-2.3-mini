...
static int decode_slice(struct AVCodecContext *avctx, void *arg)
{
    ...
    av_log(NULL, AV_LOG_FATAL, "%s,%d buf_size:%d\n",__FUNCTION__, __LINE__, (get_bits_left(&h->gb) + 7) / 8);
        ...
        for (;;) {
            ...
            /*
            av_log(NULL, AV_LOG_FATAL, "%s,%d ret:%d,h->cabac.bytestream:%p,h->cabac.bytestream_end + 4:%p,>:%d\n",__FUNCTION__, __LINE__,
                    ret,
                    h->cabac.bytestream,
                    h->cabac.bytestream_end+4,
                    h->cabac.bytestream > h->cabac.bytestream_end+4);
                    */

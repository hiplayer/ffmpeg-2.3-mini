...
static int get_last_needed_nal(H264Context *h, const uint8_t *buf, int buf_size)
{
    ...
    while(1) {
        ...
        case NAL_IDR_SLICE:
        case NAL_SLICE:
            av_log(NULL, AV_LOG_FATAL, "%s,%d bit_length:%d\n",__FUNCTION__, __LINE__,bit_length);
                            ...
                            int parse_extradata)
{
    ...
    //av_log(NULL, AV_LOG_FATAL, "%s,%d buf_size:%d\n",__FUNCTION__, __LINE__,buf_size);
    h->nal_unit_type= 0;
...

    {
        ...
        //av_log(NULL, AV_LOG_FATAL, "%s,%d next_avc:%d\n",__FUNCTION__, __LINE__,next_avc);
        nal_index     = 0;
        for (;;) {
            int consumed;
            int dst_length;
            ...
            if (buf_index >= next_avc) {
                nalsize = get_avc_nalsize(h, buf, buf_size, &buf_index);
                av_log(NULL, AV_LOG_FATAL, "%s,%d nalsize:%d\n",__FUNCTION__, __LINE__,nalsize);
                if (nalsize < 0)
                    break;
                next_avc = buf_index + nalsize;
            } else {
            ...
            //av_log(NULL, AV_LOG_FATAL, "%s,%d dst_length:%d, consumed:%d, next_avc:%d, buf_index:%d next_avc - buf_index:%d\n",__FUNCTION__, __LINE__,dst_length,consumed,next_avc, buf_index,next_avc - buf_index);
                             ...
                             int *got_frame, AVPacket *avpkt)
{
    ...
    av_log(NULL, AV_LOG_FATAL, "%s,%d avpkt->size:%d\n",__FUNCTION__, __LINE__,avpkt->size);
    h->flags = avctx->flags;

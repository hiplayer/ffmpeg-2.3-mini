                                 ...
                                 AVFilterGraph *graph_ctx)
{
	...
	av_log(NULL, AV_LOG_ERROR, "%s,%d, filt =%p\n",__FUNCTION__,__LINE__,filt);
                                             ...
                                             const char *name)
{
	...
	av_log(NULL, AV_LOG_ERROR, "%s,%d, filter =%p\n",__FUNCTION__,__LINE__,filter);
    s = ff_filter_alloc(filter, name);
    if (!s)
        return NULL;

    filters = av_realloc(graph->filters, sizeof(*filters) * (graph->nb_filters + 1));
	av_log(NULL, AV_LOG_ERROR, "%s,%d, filters =%p\n",__FUNCTION__,__LINE__,filters);
    if (!filters) {
        avfilter_free(s);
        return NULL;
    }
...
int avfilter_graph_config(AVFilterGraph *graphctx, void *log_ctx)
{
	...
	av_log(NULL, AV_LOG_ERROR, "%s,%d, graphctx =%p, log_ctx:%p\n",__FUNCTION__,__LINE__, graphctx, log_ctx);
